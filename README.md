...
## HTTP API（Phase 2：GAS 等の外部連携用）

既存の生成ロジックを薄くHTTP化したものです（生成本体は CLI と共通）。GAS 検索フロントなど
外部から呼ぶために使います。詳細設計は [docs/integration-gas-node.md](docs/integration-gas-node.md)。

```bash
# .env に RENDER_API_KEY を設定（共有シークレット）してから起動
npm run serve            # 既定 http://localhost:8080
```
...
curl -X POST http://localhost:8080/render \
  ...
  -d @samples/render-request.example.json
```

- 認証不一致/欠如は `401`、スキーマ不正は `400`、Google 認証未設定は `500` を返します。
- Slides 生成には Phase 1 と同じ Google 認証（`token.json`）が必要です。
- **公開デプロイ（Cloud Run）**: [docs/deploy-cloud-run.md](docs/deploy-cloud-run.md)。
  GAS から到達するには公開URLが必要です（`localhost` は不可）。鍵・URLはコードに書かず
  Secret Manager / 環境変数で渡します。
