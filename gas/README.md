...
- **事例生成タブ**: **管理No.は検索シートのID一覧からプルダウン選択**（選ぶと**企業名を同行B列から自動記入**）。
  画像（ロゴ/画像①②）は**ファイルアップロード可**（Drive保存→公開URL化、直接URL貼付も可）。
  **QRコードは選択した管理No.の DOC_URL から自動生成**（手入力不要）。
  「実行」→ ①**事例生成シート**へ保存(管理No.でupsert) → ②Node `/render` で生成（QRはNode内で生成）
  → ③**事例検索シート**の同じ管理No.(=ID)行の「営業資料URL」列へ生成URLを書き戻し
  → ④同ページに「完成制作物」（リンク＋プレビュー）表示。
